...
def test_endrole_ends(helpers):
    with helpers.openKeria() as (agency, agent, app, client):
        ...
        assert len(ends) == 1
        assert ends[0] == {
            "cid": "EHgwVwQT15OJvilVvW57HE4w0-GPs_Stj2OFoAHZSysY",
            "role": "agent",
        ...
        res = client.simulate_get(
            path="/endroles/EXXXVwQT15OJvilVvW57HE4w0-GPs_Stj2OFoAHZSysY"
        )
        assert res.status_code == 200
        assert len(res.json) == 0

        res = client.simulate_get(
            path="/endroles/EHgwVwQT15OJvilVvW57HE4w0-GPs_Stj2OFoAHZSysY"
        )
        assert res.status_code == 200
        ...
        res = client.simulate_get(
            path="/endroles/EHgwVwQT15OJvilVvW57HE4w0-GPs_Stj2OFoAHZSysY/agent"
        )
        assert res.status_code == 200
        assert len(res.json) == 1
        assert ends[0] == {
...
def test_locscheme_ends(helpers, mockHelpingNowUTC):
    with helpers.openKeria() as (agency, agent, app, client):
        ...
        sigs = [
            "AACOFnUk-lsVq0rLNdWCBtr51fnkXRdEzo8gnUwYF0F6xJPGL9_MXxezBc_P6e15-M1GpaHua_l3Hn4qKRMomRoM"
        ]
        body = dict(rpy=rpy.ked, sigs=sigs)

        res = client.simulate_post(
            path="/identifiers/unknown-user/locschemes", json=body
        )
        assert res.status_code == 404
        assert res.json == {
            "description": "invalid alias or prefix unknown-user",
            "title": "404 Not Found",
        }
...
def test_agent_resource(helpers, mockHelpingNowUTC):
    with helpers.openKeria() as (agency, agent, app, client):
        ...
        assert res.status_code == 404
        assert res.json == {
            "description": "no agent for bad_pre",
            "title": "404 Not Found",
        }
        ...
        body = {"rot": "fake_rot", "sigs": "fake_sigs"}
        res = client.simulate_put(path=f"/agent/{agent.caid}", body=json.dumps(body))
        assert res.status_code == 400
        assert res.json == {
        ...
        body = {"rot": "fake_rot", "sigs": "fake_sigs", "sxlt": "fake_sxlt"}
        res = client.simulate_put(path=f"/agent/{agent.caid}", body=json.dumps(body))
        assert res.status_code == 400
        assert res.json == {
...
def test_identifier_collection_end(helpers):
    ...
    with (
        ...
        sigers = [
            signer1.sign(ser=rserder.raw, index=0).qb64,
        ...
        res = client.simulate_post(
            path="/identifiers/multisig/events", body=json.dumps(body)
        )
        assert res.status_code == 200
        ...
        assert group["keys"] == [
            "DKy7HNSig5OuFOfXKq85H_R35KA6TqjRnaLKrCl8sYih",
            "DNHAT44MX_0kvxXm2JParu_VXDk6_Y3csi22h85ZXklA",
            "DFWjujqjGJDJPDTxcCUv5uQ7rfHHVKM8FWIVMrqMM6ib",
        ]
        assert group["ndigs"] == [
            "EHTQYGOEz97iOPC_DgEEGncSa1-X-TDiHdGNKQiN_3XJ",
    ...
    with helpers.openKeria() as (agency, agent, app, client):
        ...
        nxts = params["nxts"]
        signers = [
            decrypter.decrypt(cipher=core.Cipher(qb64=nxt), transferable=True)
            for nxt in nxts
        ]
        ...
        sigers = [signer.sign(ser=serder.raw, index=0).qb64 for signer in signers]
        body = {"ixn": serder.ked, "sigs": sigers}
        res = client.simulate_post(
            path="/identifiers/randy1/events", body=json.dumps(body)
        )
        assert res.status_code == 200
        ...
        # Bad interactions
        res = client.simulate_post(
            path="/identifiers/badrandy/events", body=json.dumps(body)
        )
        assert res.status_code == 404
        ...
        res = client.simulate_post(
            path="/identifiers/randybad/events", body=json.dumps(body)
        )
        assert res.status_code == 404
        ...
        assert res.status_code == 400
        assert res.json == {
            "description": "unknown witness EJJR2nmwyYAZAoTNZH3ULvaU6Z-i0d8fSVPzhzS6b5CM",
            "title": "400 Bad Request",
        }
...
def test_challenge_ends(helpers):
    with helpers.openKeria() as (agency, agent, app, client):
        ...
        exn, _ = exchanging.exchange(
            route="/challenge/response", payload=payload, sender=agent.agentHab.pre
        )
        ...
        result = client.simulate_put(
            path="/challenges/henk/verify/Eo6MekLECO_ZprzHwfi7wG2ubOt2DWKZQcMZvTbenBNU",
            body=b,
        )
        ...
        b = json.dumps(data).encode("utf-8")
        result = client.simulate_put(
            path="/challenges-verify/EFt8G8gkCJ71e4amQaRUYss0BDK4pUpzKelEIr3yZ1D0",
            body=b,
        )
        ...
        data = dict(words=words, said=exn.said)
        b = json.dumps(data).encode("utf-8")
        result = client.simulate_post(
            path=f"/challenges/henk/verify/{aid['i']}", body=b
        )
...
def test_contact_ends(helpers):
    with (
        helpers.openKeria() as (agency, agent, app, client),
        habbing.openHby(
            name="ken", salt=core.Salter(raw=b"0123456789ghijkl").qb64
        ) as kenHby,
    ):
        ...
        response = client.simulate_post(
            "/contacts/E8AKUcbZyik8EdkOwXgnyAxO5mSIPJWGZ_o7zMhnNnjo/pal", body=b
        )
        ...
        for i in range(5):
            data = dict(id=aid[i], first=f"Ken{i}", last=f"Burns{i}", company="GLEIF")
            b = json.dumps(data).encode("utf-8")
            response = client.simulate_post(f"/contacts/{aids[i]}", body=b)
            assert response.status == falcon.HTTP_200

        # Dupplicate
        data = dict(id=aid[0], first=f"Ken{0}", last=f"Burns{0}", company="GLEIF")
        b = json.dumps(data).encode("utf-8")
        response = client.simulate_post(f"/contacts/{aids[i]}", body=b)
        assert response.status == falcon.HTTP_400

        response = client.simulate_get(
            "/contacts/E8AKUcbZyik8EdkOwXgnyAxO5mSIPJWGZ_o7zMhnNnjo"
        )
        assert response.status == falcon.HTTP_404
        ...
        response = client.simulate_put(
            "/contacts/E8AKUcbZyik8EdkOwXgnyAxO5mSIPJWGZ_o7zMhnNnjo", body=b
        )
        assert response.status == falcon.HTTP_404
        ...
        response = client.simulate_get(
            "/contacts", query_string="group=company&filter_value=Pro"
        )
        assert response.status == falcon.HTTP_200
        ...
        response = client.simulate_get(
            "/contacts", query_string="filter_field=last&filter_value=Burns3"
        )
        assert response.status == falcon.HTTP_200
        assert response.json == [
            {
        ...
        response = client.simulate_get(
            "/contacts", query_string="filter_field=last&filter_value=Burns"
        )
        assert response.status == falcon.HTTP_200
        assert response.json == [
            {
        ...
        response = client.simulate_post(
            "/contacts/E8AKUcbZyik8EdkOwXgnyAxO5mSIPJWGZ_o7zMhnNnjo/img",
            body=data,
            headers=headers,
        )
        assert response.status == falcon.HTTP_404
        ...
        response = client.simulate_post(
            f"/contacts/{aids[0]}/img", body=data, headers=headers
        )
        assert response.status == falcon.HTTP_400
...
def test_identifier_resource_end(helpers):
    with (
        ...
        assert res.status_code == 404
        assert res.json == {
            "description": "bad is not a valid identifier name or prefix",
            "title": "404 Not Found",
        }
        ...
        res = client.simulate_get(
            path="/identifiers/EHgwVwQT15OJvilVvW57HE4w0-GPs_Stj2OFoAHZSysY"
        )
        assert res.status_code == 200
        assert res.json["prefix"] == "EHgwVwQT15OJvilVvW57HE4w0-GPs_Stj2OFoAHZSysY"
...
def test_approve_delegation(helpers):
    ...
    with habbing.openHby(name="keria", salt=salter.qb64, temp=True, cf=cf) as hby:
        ...
        serder, signers = helpers.incept(
            bran=b"abcdefghijk0123456789", stem="signify:controller", pidx=0
        )
        ...
        serder = eventing.interact(
            pre=ctrl["state"]["i"], sn="1", dig=ctrl["state"]["d"], data=[anchor]
        )
        body = {
            "ixn": serder.ked,
        ...
        res = client.simulate_put(
            path=f"/agent/{controllerAID}?type=ixn", body=json.dumps(body)
        )
...
def test_rotation(helpers):
    ...
    with (
        ...
        datetime.fromisoformat(
            icp_dt
        )  # will raise an error if the format is not a valid iso dt
        ...
        # Rotate aid1
        bodyrot1 = helpers.createRotate(
            aid1, salt, signers1, pidx=0, ridx=1, kidx=1, wits=[], toad=0
        )
        res = client.simulate_post(
            path=f"/identifiers/{aid1['name']}/events", body=json.dumps(bodyrot1)
        )
        assert res.status_code == 200
