...
def test_public_status_route_renders_live_tel_state(helpers, monkeypatch, ilk, revoked):
    aid = "E" + ("A" * 43)
    with helpers.openKeria() as (agency, agent, app, client):
        ...
        monkeypatch.setattr(
            w3cing,
            "registryTever",
            lambda _agent, _registry_said: SimpleNamespace(vcState=lambda _said: state),
        )
        ...
        assert (
            result.json["id"] == "http://status.example/w3c/vc/status/credential-said"
        )
...
def test_projection_doer_advances_signatures_and_posts_verifier(helpers, monkeypatch):
    with helpers.openKeria() as (_agency, agent, _app, _client):
        patch_projection_inputs(monkeypatch, agent)
        session = w3cing.createProjectionSession(
            agent,
        ...
        assert (
            agent.adb.w3creq.get(keys=(proof_request.d,)).state
            == w3cing.W3C_REQ_COMPLETE
        )
