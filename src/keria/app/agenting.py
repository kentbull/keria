...
def agencyDoist(doers: List[Doer]):
    ...
    doers.append(
        GracefulShutdownDoer(agency=agency, tock=agency.keriaTocks["gracefulShutdown"])
    )
    doist.doers = doers
    return doist
...
class Agency(doing.DoDoer):
    ...
    def __init__(
        ...
        configuration = self.cf.get() if self.cf is not None else {}
        resolvedTocks = tocking.resolveTocks(configuration)
        self.keriTocks = resolvedTocks.keri
        self.keriaTocks = resolvedTocks.keria
        ...
        super(Agency, self).__init__(
            doers=[
                Releaser(
                    ...
                    tock=self.keriaTocks["releaser"],
                )
            ],
            tock=self.keriaTocks["agency"],
        )
...
class Agent(doing.DoDoer):
    ...
    def __init__(self, hby, rgy, agentHab, agency, caid, tocks=None, **opts):
        ...
        if tocks is None:
            self.tocks = tocking.resolveTocks(dict(self.cfd), environ={}).keria
        else:
            self.tocks = tocks
        ...
        super(Agent, self).__init__(doers=doers, tock=self.tocks["agent"], **opts)
...
def createBootServerDoer(config: KERIAServerConfig, agency: Agency):
    ...
    return http.ServerDoer(server=bootServer, tock=agency.keriaTocks["bootServer"])
...
def createAdminServerDoer(config: KERIAServerConfig, agency: Agency):
    ...
    return adminApp, http.ServerDoer(
        server=adminServer, tock=agency.keriaTocks["adminServer"]
    )
...
def createHttpServerDoer(
    ...
    return http.ServerDoer(server=server, tock=agency.keriaTocks["httpServer"])
...
class Submitter(doing.DoDoer):
    def __init__(self, hby, submits, witRec, tock=0.0):
        """
        ...
        super(Submitter, self).__init__(always=True, tock=self.tock)
