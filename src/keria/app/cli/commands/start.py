...
parser.add_argument(
    ...
    dest="bran",
    default=os.getenv("KERIA_PASSCODE"),
)  # passcode => bran
parser.add_argument(
    "--config-file", dest="configFile", action="store", help="configuration filename"
)
parser.add_argument(
    "--config-dir",
    dest="configDir",
...
parser.add_argument(
    ...
    action="store",
    required=False,
    default=os.getenv("KERIA_LOG_LEVEL", "CRITICAL"),
...
def launch(args):
    agenting.runAgency(
        agenting.KERIAServerConfig(
            ...
            w3c=w3cing.configFromEnvironment(),
