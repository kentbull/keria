...
KERIA owns the observable readiness projection for did:webs assets. Edge
controllers create the backing registry and designated-alias credential with
normal Signify APIs, then KERIA validates that state before publishing compact
readiness.
"""
...
def _loadJsonResource(filename: str) -> dict[str, Any]:
    with (
        ...
        .joinpath(filename)
        .open(encoding="utf-8") as resource
    ):
...
def setupInfo(agent, config: DidWebsConfig, name: str, aid: str) -> dict:
    """Return did:webs setup state and edge-call arguments for one managed AID."""
    if not config.enabled:
        raise ArtifactUnavailable("did:webs is not enabled")
    ...
    if credential is not None:
        credential_said = getattr(credential, "said", None) or credential.sad.get("d")

    ready = bool(matchingDesignatedAliases(agent.hby, agent.rgy, aid, did))
    issue_args = None
    if registry_id is not None:
        issue_args = {
            "ri": registry_id,
            "s": DES_ALIASES_SCHEMA,
            "a": aliasCredentialData(config, aid),
            "r": copy.deepcopy(DES_ALIASES_RULES),
        }
...
def matchingDesignatedAliases(
    ...
    """Return configured designated aliases only when the full alias set exists."""
    wanted = {did, did.replace("did:webs", "did:web", 1)}
    aliases = genDesignatedAliases(hby, rgy, aid)
    if not wanted.issubset(set(aliases)):
        return []
